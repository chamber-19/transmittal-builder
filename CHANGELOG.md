...
- Bumped `@chamber-19/desktop-toolkit` pin to v2.2.7, which fixes the broken
  ...
  `hooks.nsh` was killing the updater shim mid-install. The v2.2.7 source
  pin landed in v6.2.6, but the build artifact did not actually carry
  v2.2.7's code — see v6.2.7 below for the actual delivery.
